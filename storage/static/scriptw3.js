...
/*shipment1*/
if (localStorage.shipment5 != null) {
    shipment14 = JSON.parse(localStorage.shipment5);
    numm = shipment14.length;
    for(i=0;i<shipment14.length;i++){
        t4+=parseInt(shipment14[i].pro)
    }
}
else {
    shipment14 = [];
...
function addsh() {
    ...
    localStorage.setItem('shipment5', JSON.stringify(shipment14));
    show_shimpent1();
}
function show_shimpent1() {
    let table = "";
    for (let i = 0; i < shipment14.length; i++) {
        table += '<tr><td>' + shipment14[i].num + '</td><td> ' + shipment14[i].amount + '</td><td> ' + shipment14[i].type + ' </td><td> ' + shipment14[i].price + ' </td><td> ' + shipment14[i].expe + ' </td><td> ' + shipment14[i].pro + ' </td><td><input type="button"class="btn" value="edit" onclick="edit_sh(' + i + ')"></td></tr>';
    }
