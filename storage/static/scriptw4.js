...
/*shipment1*/
if (localStorage.shipment6 != null) {
    shipment15 = JSON.parse(localStorage.shipment6);
    numm = shipment15.length;
    for(i=0;i<shipment15.length;i++){
        t5+=parseInt(shipment15[i].pro)
    }
}
else {
    shipment15 = [];
...
function addsh() {
   ...
   let amount = document.getElementById('am').value;
    ...
    localStorage.setItem('shipment6', JSON.stringify(shipment15));
    show_shimpent1();
}
