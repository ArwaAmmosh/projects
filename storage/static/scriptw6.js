...
/*shipment1*/
if (localStorage.shipment8 != null) {
    shipment17 = JSON.parse(localStorage.shipment8);
    numm = shipment17.length;
    for(i=0;i<shipment17.length;i++){
        t1+=parseInt(shipment17[i].pro)
    }
}
else {
    shipment17 = [];
...
function addsh() {
    ...
    let rent = document.getElementById('ren').value;

    let pro = (sell * amount) - expe - price-(rent*amount);
    ...
    localStorage.setItem('shipment8', JSON.stringify(shipment17));
    show_shimpent1();
}
...
function edit_sh(i) {
    temp = i;
    document.getElementById('am').value = shipment17[i].date;
